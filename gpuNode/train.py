...
def set_args():
    ...
    parser.add_argument('--device', default=[0,1], type=list, help='设置训练或测试时使用的显卡')
    ...
    parser.add_argument('--train_file_path', default='./data_dir/chinese-news-title/train.json', type=str, help='新闻标题生成的训练数据')
    parser.add_argument('--test_file_path', default='./data_dir/chinese-news-title/test.json', type=str, help='新闻标题生成的测试数据')
    parser.add_argument('--pretrained_model_path', default=None, type=str, help='预训练的GPT2模型的路径')
    parser.add_argument('--data_dir', default='./data_dir/chinese-news-title', type=str, help='生成缓存数据的存放路径')
    ...
    parser.add_argument('--output_dir', default='output_dir_news/', type=str, help='模型输出路径')
...
def main():
    ...
    os.environ["CUDA_VISIBLE_DEVICE"] = ",".join(str(args.device))
    # 获取device信息，用于模型训练
    device = torch.device("cuda" if torch.cuda.is_available() and len(args.device) >= 0 else "cpu")
    print("device:", device)
    # 设置随机种子，方便模型复现
    if args.seed:
