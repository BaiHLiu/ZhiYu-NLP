...
def set_args():
    ...
    parser.add_argument('--port', default=12345, type=int, help='设置监听端口')
    parser.add_argument('--device', default='0', type=str, help='设置预测时使用的显卡,使用CPU设置成-1即可')
    parser.add_argument('--model_path', default='output_dir/checkpoint-12056', type=str, help='模型文件路径')
...
def main():
    ...
    # model.to(device)
    # model.eval()
    json_ = str(request.values.get('content'))
    if json_ == 'None':
        return jsonify({'summary': 'error: 未输入内容'})
    titles = predict_one_sample(model, tokenizer, device, args, json_)
    return jsonify({'summary': titles})
...
if __name__ == '__main__':
    ...
    app.run(host='192.168.214.146', port=args.port, debug=False)
